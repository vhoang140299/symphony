...
test("rejects unsafe pagination Links and detects loops", async () => {
  ...
  for (const { link, error } of cases) {
    ...
    await assert.rejects(tracker.fetchIssuesByStates(["open"]), error);
  }
  ...
  await assert.rejects(looping.fetchIssuesByStates(["open"]), /pagination Link loop detected/);
...
test("validates configuration, states, ids, payloads, and redacts tokens from errors", async () => {
  assert.throws(() => new GitHubTracker({ repo: "widget" }), /provider\.owner must be a non-empty string/);
  assert.throws(
