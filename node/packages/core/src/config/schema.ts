...
const codexCompatibilitySchema = z
  .object({
    command: nonEmptyString
      .default("codex app-server")
      .refine((value) => value === "codex app-server", "Node supports only codex.command: codex app-server"),
    approval_policy: nonEmptyString
      .default("never")
      .refine((value) => value === "never", "Node requires codex.approval_policy: never"),
    thread_sandbox: nonEmptyString
      .default("workspace-write")
      .refine((value) => value === "workspace-write", "Node requires codex.thread_sandbox: workspace-write"),
    turn_timeout_ms: positiveInteger.default(3_600_000),
...
const rawWorkflowConfigSchema = z
  .object({
    ...
    codex: codexCompatibilitySchema.optional(),
  ...
  .passthrough()
  .superRefine((value, context) => {
    if (value.codex !== undefined && value.runtime !== undefined) {
      context.addIssue({
        code: "custom",
        path: ["codex"],
        message: "Top-level codex configuration cannot be combined with runtime configuration",
      });
    }
...
export function parseWorkflowConfig(input: unknown): WorkflowConfig {
  ...
  const runtime = parsed.runtime ?? runtimeSchema.parse({});
  const requiredLabels = parsed.tracker.required_labels.map((label) => label.trim().toLowerCase());
  const stateLimits = Object.fromEntries(
    Object.entries(parsed.agent.max_concurrent_agents_by_state).flatMap(([state, limit]) => {
      const normalizedState = state.trim().toLowerCase();
      return normalizedState !== "" && typeof limit === "number" && Number.isSafeInteger(limit) && limit > 0
  ...
  return {
    tracker: {
      ...
      activeStates: parsed.tracker.active_states
        ?? (parsed.tracker.kind === "github" ? ["open"] : ["Todo", "In Progress"]),
      terminalStates:
        parsed.tracker.terminal_states ??
        (parsed.tracker.kind === "github"
          ? ["closed"]
    ...
    runtime: parsed.codex === undefined
      ? {
          kind: runtime.kind,
      ...
      : {
          kind: "codex",
          turnTimeoutMs: parsed.codex.turn_timeout_ms,
          stallTimeoutMs: parsed.codex.stall_timeout_ms,
          options: { read_timeout_ms: parsed.codex.read_timeout_ms },
        },
