...
import { createLogger, type AppLogger } from "@ai-symphony/core/log.js";
...
import { workspaceKey } from "@ai-symphony/core/workspace/manager.js";
...
test("dispatches a prototype-named state without an explicit per-state limit", async () => {
  ...
  const workflowPath = await writeWorkflow(directory, [issue], { activeStates: ["constructor"] });
  const orchestrator = new Orchestrator(new WorkflowStore(workflowPath, logger), logger, { tracker, driver });
...
test("a partial host-delivery failure waits for reloaded delivery config without rerunning the agent", async () => {
  ...
  await writeDeliveryWorkflow(directory, "claude", { delivery: false, maxAttempts: 1 });
  ...
  assert.deepEqual(commentKeys, [deliveryKey]);
  assert.deepEqual(compactState(orchestrator), { running: 0, retrying: 1, blocked: 0 });

  await writeDeliveryWorkflow(directory, "claude", { maxAttempts: 1 });
  const repaired = new Date(Date.now() + 4_000);
  await utimes(workflowPath, repaired, repaired);
  now += 60_000;
  await orchestrator.pollOnce();
  await orchestrator.waitForCurrentRuns();
...
test("a blocked retry label uses the reloaded control config", async () => {
  ...
  assert.deepEqual(operations, ["run:1"]);
  assert.deepEqual(current.labels, ["symphony", "ReTrY-Me"]);
  assert.deepEqual(compactState(orchestrator), { running: 0, retrying: 0, blocked: 1 });

  current = { ...current, labels: [...current.labels, "Replacement-Label"] };
  await orchestrator.pollOnce();
  await orchestrator.waitForCurrentRuns();

  assert.equal(runs, 2);
  assert.deepEqual(operations, ["run:1", "remove:replacement-label", "run:2"]);
  assert.deepEqual(current.labels, ["symphony", "ReTrY-Me"]);
  assert.equal(operations.filter((operation) => operation === "remove:replacement-label").length, 1);
...
test("full capacity batches running reconciliation without fetching candidates", async () => {
  ...
  const tracker: Tracker = {
    async fetchIssuesByStates(states) {
      candidateFetches += 1;
      return [...issues.values()].filter((issue) => states.includes(issue.state));
    },
    async fetchIssuesByIds(ids) {
      refreshes.push([...ids]);
  ...
  const workflowPath = await writeWorkflow(directory, [
    rawIssue(first.id, first.identifier, first.priority ?? 1, first.createdAt ?? "2025-01-01T00:00:00Z"),
    rawIssue(second.id, second.identifier, second.priority ?? 2, second.createdAt ?? "2025-01-02T00:00:00Z"),
  ], { maxConcurrentAgents: 2 });
  const orchestrator = new Orchestrator(new WorkflowStore(workflowPath, logger), logger, { tracker, driver });
...
test("hot reload applies current routing to a queued retry", async () => {
...
test("hot reload gives a queued retry the current tracker adapter without resetting its attempt", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "symphony-reload-retry-tracker-"));
  const issue = rawIssue("adapter", "ADAPTER-1", 1, "2025-01-01T00:00:00Z");
  const contexts: Array<{ attempt: number | null; title: string }> = [];
  const driver = new FakeDriver(async function* (context) {
    contexts.push({ attempt: context.attempt, title: context.issue.title });
    yield event("turn_failed", { summary: "queue a retry" });
  });
  const workflowPath = await writeWorkflow(directory, [issue], { maxAttempts: 2 });
  let now = Date.UTC(2026, 0, 1);
  ...
  const unrelated = { ...rawIssue("other", "OTHER-1", 2, "2025-01-02T00:00:00Z"), labels: [] };
  await writeWorkflow(directory, [{ ...issue, title: "Reloaded issue" }, unrelated], { maxAttempts: 2 });
  const later = new Date(Date.now() + 2_000);
  await utimes(workflowPath, later, later);
  now += 100;
  await orchestrator.pollOnce();
  ...
  assert.deepEqual(contexts, [
    { attempt: null, title: "Issue ADAPTER-1" },
    { attempt: 1, title: "Reloaded issue" },
  ]);
  assert.deepEqual(compactState(orchestrator), { running: 0, retrying: 0, blocked: 1 });
  await orchestrator.stop();
});

test("hot reload applies the current retry budget when an in-flight run finishes", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "symphony-reload-retry-budget-"));
  const issue = rawIssue("budget", "BUDGET-1", 1, "2025-01-01T00:00:00Z");
  const tracker = new MemoryTracker({ issues: [issue] });
  const firstStarted = deferred<void>();
  ...
  const driver = new FakeDriver(async function* (context) {
    ...
    if (runs === 1) {
      ...
      yield event("turn_failed", { summary: "retry with the current budget" });
      return;
    }
    tracker.setIssueState(context.issue.id, "Done");
    yield event("turn_completed");
  });
  const workflowPath = await writeWorkflow(directory, [issue], { maxAttempts: 1 });
  let now = Date.UTC(2026, 0, 1);
  const orchestrator = new Orchestrator(new WorkflowStore(workflowPath, logger), logger, {
    tracker,
    driver,
  ...
  await orchestrator.pollOnce();
  await firstStarted.promise;
  await writeWorkflow(directory, [issue], { maxAttempts: 2 });
  const later = new Date(Date.now() + 2_000);
  await utimes(workflowPath, later, later);
  await orchestrator.pollOnce();
...
test("tracker reload cannot retarget running or blocked work with a repo-local id", async () => {
  ...
  const firstStarted = deferred<void>();
  const releaseFirst = deferred<void>();
  const seen: Array<{ identifier: string; attempt: number | null; sessionId: string | undefined }> = [];
  let runs = 0;
  const driver = new FakeDriver(async function* (context) {
    runs += 1;
    seen.push({
  ...
  await orchestrator.pollOnce();
  await orchestrator.waitForCurrentRuns();
  assert.equal(orchestrator.snapshot().blocked[0]?.identifier, "NEW-1");
  assert.equal(orchestrator.snapshot().blocked[0]?.reasonCode, "operator_action_required");

  assert.equal(await orchestrator.retryBlocked("OLD-1"), false);
  assert.deepEqual(seen, [
    { identifier: "OLD-1", attempt: null, sessionId: undefined },
    { identifier: "NEW-1", attempt: null, sessionId: undefined },
  ]);
  await orchestrator.stop();
});
...
async function writeWorkflow(
  ...
  const config = {
    tracker: {
      ...
      active_states: options.activeStates ?? ["Todo", "In Progress"],
      terminal_states: ["Done"],
    },
