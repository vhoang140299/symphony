...
import {
  ...
  parseCodexRuntimeOptions,
  resolveSafeCodexHome,
  type CodexStreamRequest,
} from "../src/codex.js";
import { createAgentDriver } from "../src/registry.js";
import { agentCompletionOutputSchema } from "@ai-symphony/core/completion.js";
import type { AgentEvent, AgentRunContext, Issue } from "@ai-symphony/core/domain.js";
...
test("bounds the first Codex response and clears that timeout after startup", async () => {
  let timedOutSignal: AbortSignal | undefined;
  const timedOut = new CodexAgentDriver(async (request) => {
    timedOutSignal = request.signal;
    return waitForAbort(request.signal);
  });

  const timeoutEvents = await collect(timedOut.run(context({ runtimeOptions: { read_timeout_ms: 10 } })));
  assert.equal(timedOutSignal?.aborted, true);
  assert.equal(timeoutEvents.filter(({ type }) => type === "turn_failed").length, 1);
  assert.match(timeoutEvents[0]?.summary ?? "", /Codex startup timed out after 10ms/);

  const active = new CodexAgentDriver(async (request) => activeStream(request.signal));
  const activeEvents = await collect(active.run(context({ runtimeOptions: { read_timeout_ms: 5 } })));
  assert.equal(activeEvents.at(-1)?.type, "turn_completed");

  async function* waitForAbort(signal: AbortSignal): AsyncIterable<ThreadEvent> {
  ...
  async function* activeStream(signal: AbortSignal): AsyncIterable<ThreadEvent> {
    yield { type: "thread.started", thread_id: "thread-active" };
...
test("validates Codex read timeout options", () => {
  assert.equal(parseCodexRuntimeOptions({}).read_timeout_ms, undefined);
  assert.equal(parseCodexRuntimeOptions({ read_timeout_ms: 25 }).read_timeout_ms, 25);
  assert.throws(() => parseCodexRuntimeOptions({ read_timeout_ms: 0 }));
  assert.throws(() => parseCodexRuntimeOptions({ read_timeout_ms: 1.5 }));
});
...
test("Codex wrapper forces user config, rules, provider tools, and extension layers off", async () => {
  ...
  const wrapperPath = fileURLToPath(new URL("../codex-symphony-wrapper.mjs", import.meta.url));
...
`,
    ...
    await chmod(fakeCodex, 0o700);
    const wrapper = spawn(fileURLToPath(new URL("../codex-symphony-wrapper.mjs", import.meta.url)), ["exec"], {
      env: {
        ...process.env,
        SYMPHONY_CODEX_REAL_EXECUTABLE: fakeCodex,
        CODEX_WRAPPER_CAPTURE: capturePath,
