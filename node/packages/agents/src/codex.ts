...
export class CodexAgentDriver implements AgentDriver {
  ...
  constructor(
    ...
    private readonly wrapperPath = codexWrapperPath,
  ) {}
  ...
  async *run(context: AgentRunContext): AsyncIterable<AgentEvent> {
    ...
    try {
      ...
      const readTimeoutController = configured.read_timeout_ms === undefined
        ? undefined
        : new AbortController();
      const streamSignal = readTimeoutController === undefined
        ? context.signal
        : AbortSignal.any([context.signal, readTimeoutController.signal]);
      ...
      if (readTimeoutSummary !== undefined) streamSignal.throwIfAborted();
...
async function assertCodexHomeOutsideWorkspace(codexHome: string, workspacePath: string): Promise<void> {
  try {
    const workspace = await realpath(workspacePath);
    if (isPathContained(codexHome, workspace) || isPathContained(workspace, codexHome)) {
...
export const codexWrapperPath = fileURLToPath(new URL("../codex-symphony-wrapper.mjs", import.meta.url));
