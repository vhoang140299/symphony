...
const exampleWorkflowPath = fileURLToPath(new URL("../../../workflows/WORKFLOW.md", import.meta.url));
...
test("preflight and doctor accept the safe top-level codex profile", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "symphony-cli-codex-profile-"));
  const workflowPath = path.join(directory, "WORKFLOW.md");
  await writeFile(
    workflowPath,
  ...
  try {
    const preflight = await runPreflight(workflowPath);
    assert.equal(preflight.tracker, "memory");
    assert.equal(preflight.runtime, "codex");
    assert.equal(preflight.fetched, 0);

    const report = await runDoctor(workflowPath);
    assert.equal(report.tracker, "memory");
