...
- Trackers: in-memory issues; Linear polling with manual Claude tools, host-controlled handoff, and
  operator retry labels through the official SDK; GitHub Issues polling with manual Claude tools,
...
> **The release pipeline is currently disabled.** Splitting this implementation into a pnpm
> workspace made every package private, so no new tarballs are published and the packaging smoke
> test was removed. Releases up to and including `node-v0.6.0` are still downloadable, but for
> current code use [Build from source](#build-from-source) instead. This section is kept for when
...
pnpm run build
pnpm start ./workflows/WORKFLOW.md
```
...
```bash
node packages/cli/dist/cli.js --once ./workflows/WORKFLOW.md
```
...
```bash
node packages/cli/dist/cli.js --preflight ./workflows/WORKFLOW.md
```
...
```bash
node packages/cli/dist/cli.js --doctor ./workflows/WORKFLOW.md
```
...
```bash
node packages/cli/dist/cli.js --port 3000 --http-host 127.0.0.1 ./workflows/WORKFLOW.md
```
...
```bash
pnpm run check
pnpm --filter @ai-symphony/dashboard run build
pnpm test
pnpm test:watch
```
...
Copy [`WORKFLOW.github.md`](../../workflows/WORKFLOW.github.md) to a local profile:

```bash
cp workflows/WORKFLOW.github.md WORKFLOW.preview.claude.md
```
...
GITHUB_TOKEN="$(gh auth token)" \
  node packages/cli/dist/cli.js --once ./WORKFLOW.preview.claude.md
```
...
To use Codex instead, copy [`WORKFLOW.codex.github.md`](../../workflows/WORKFLOW.codex.github.md), make the same
...
```bash
cp workflows/WORKFLOW.codex.github.md WORKFLOW.preview.codex.md
...
GITHUB_TOKEN="$(gh auth token)" CODEX_HOME="$SYMPHONY_CODEX_HOME" \
  node packages/cli/dist/cli.js --once ./WORKFLOW.preview.codex.md
```
...
[`WORKFLOW.md`](../../workflows/WORKFLOW.md) is both configuration and prompt. YAML front matter controls the
...
invalid reload keeps the last known-good configuration. A running agent keeps the configuration it
started with, while queued retries and blocked/manual retries revalidate against the current tracker
and launch with the reloaded configuration. Pending host delivery also uses the current validated
host adapter while retaining its completion and idempotency key; if a reload removes the required
delivery capability or changes tracker scope, the claim waits for configuration repair.
...
[`WORKFLOW.github.md`](../../workflows/WORKFLOW.github.md) and
[`WORKFLOW.codex.github.md`](../../workflows/WORKFLOW.codex.github.md) are host-delivery GitHub issue-to-PR profiles
for Claude and Codex. Copy one before editing; the default `WORKFLOW.md` remains an inert local
example.

### Tracker adapter profiles

The compact implementation profiles for Memory, Linear, and GitHub—including exact provider keys,
scope and limits, issue normalization, model-facing tools, and portable error mapping—live in the
[`@ai-symphony/trackers` guide](../trackers/README.md). This guide keeps the operational workflow
examples below.
...
Workflows shared with the reference Symphony implementation may instead use its safe top-level
Codex profile:
...
This compatibility form maps to the SDK runtime. It accepts only the exact command and locked-down
policies shown above; free-form commands, other approval or thread-sandbox values, and
`turn_sandbox_policy` are rejected. Do not configure both `codex:` and `runtime:`. The read timeout
adds a startup bound through the first SDK event; the overall turn and stall timers remain active
for the whole run. Keep using `runtime.kind: codex` when you need Node-specific options such as
model, reasoning effort, environment allowlisting, or an executable override.
...
`model_reasoning_effort`, `skip_git_repo_check`, `read_timeout_ms`, `env_allowlist`, and
`codex_executable`. The default environment preserves the local Codex login and basic process/TLS
variables while stripping unrelated host secrets. Add `CODEX_API_KEY` or `OPENAI_API_KEY` to
`env_allowlist` only when that credential mode is intentional.
...
Additional trackers implement the `Tracker` contract under `packages/trackers/src/` and register their
`kind` in the tracker registry. Additional coding agents follow the same pattern with the
`AgentDriver` contract under `packages/agents/src/`.
