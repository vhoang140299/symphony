...
import { createOperationsServer } from "@ai-symphony/server/http/server.js";
import { createLogger } from "@ai-symphony/core/log.js";
import { Orchestrator } from "@ai-symphony/server/orchestrator.js";
...
async function main(args: string[]): Promise<void> {
  ...
  const logger = createLogger(undefined, once || preflight ? process.stderr : undefined);
  if (preflight) {
    process.stdout.write(`${JSON.stringify(await runPreflight(workflowPath, logger))}\n`);
    return;
  }
