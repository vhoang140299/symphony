...
import { LinearError, LinearErrorType, type LinearClientOptions } from "@linear/sdk";
import {
  LinearTracker,
...
import { TrackerError } from "../src/error.js";
import { createTracker as createRegisteredTracker, validateTrackerProvider } from "../src/registry.js";
...
test("validates provider settings offline, resolves env keys at runtime, and skips empty reads", async () => {
  ...
  try {
    ...
    assert.throws(
      ...
      trackerError("missing_tracker_secret", /LINEAR_API_KEY is not set/),
    );
    assert.throws(
  ...
  assert.throws(
    ...
    trackerError("invalid_tracker_config", /provider\.project_slug/),
  );
  assert.throws(
...
test("registers Linear tracker construction and count-only malformed-candidate warnings", async () => {
  ...
  process.env[TEST_KEY_ENVIRONMENT] = "test-secret";
  globalThis.fetch = async () => new Response(JSON.stringify(issuePage([
    rawIssue("valid"),
    rawIssue("bad-1", { title: " ", providerSecret: "secret-provider-payload-1" }),
    rawIssue("bad-2", { id: null, providerSecret: "secret-provider-payload-2" }),
  ])), {
    status: 200,
  ...
  try {
    ...
    const tracker = createRegisteredTracker("linear", provider, {
      logger,
      terminalStates: ["Released"],
    });
    assert.ok(tracker instanceof LinearTracker);
    assert.equal(tracker.issueStateMutationMode, "named");
    assert.deepEqual((await tracker.fetchIssuesByStates(["Todo"])).map(({ id }) => id), ["valid"]);
    assert.deepEqual(warnings, [{
      bindings: { malformed_count: 2 },
      message: "Dropping malformed Linear issue records",
    }]);
...
test("batches opaque ID refreshes, deduplicates requests, and restores requested order", async () => {
  ...
  await assert.rejects(
    malformed.fetchIssuesByIds(["issue-1"]),
    trackerError("tracker_response", /malformed issue during ID refresh/),
  );
});
...
test("rejects malformed or looping cursors and SDK failures without leaking the API key", async () => {
  ...
  await assert.rejects(
    missing.fetchIssuesByStates(["Todo"]),
    trackerError("tracker_pagination", /missing endCursor/),
  );
  ...
  await assert.rejects(
    looping.fetchIssuesByStates(["Todo"]),
    trackerError("tracker_pagination", /cursor loop detected/),
  );
  ...
  await assert.rejects(failed.fetchIssuesByStates(["Todo"]), (error: unknown) => {
    trackerError("tracker_request")(error);
    assert.ok(error instanceof Error);
  ...
  try {
    ...
    await assert.rejects(sdkTracker.fetchIssuesByStates(["Todo"]), (error: unknown) => {
      trackerError("tracker_response")(error);
      assert.ok(error instanceof Error);
      assert.equal(error.message, "Linear GraphQL request failed");
      assert.doesNotMatch(error.message, new RegExp(secret));
      return true;
...
test("maps Linear SDK HTTP and rate-limit errors without exposing provider details", async () => {
  const cases = [
    {
      category: "tracker_status" as const,
      error: new LinearError(
        { message: "secret 503 detail", response: { status: 503 } },
        undefined,
        LinearErrorType.NetworkError,
      ),
    },
    {
      category: "tracker_rate_limited" as const,
      error: new LinearError(
        { message: "secret rate detail", response: { status: 429 } },
        undefined,
        LinearErrorType.Ratelimited,
      ),
    },
    {
      category: "tracker_response" as const,
      error: new LinearError(
        { message: "secret GraphQL detail", response: { status: 200 } },
        undefined,
        LinearErrorType.NetworkError,
      ),
    },
    {
      category: "tracker_status" as const,
      error: new TrackerError("tracker_status", "secret dependency detail"),
    },
  ];

  for (const { category, error } of cases) {
    const tracker = createTracker(() => { throw error; }).tracker;
    await assert.rejects(
      tracker.fetchIssuesByStates(["Todo"]),
      trackerError(category, /^Linear GraphQL request failed$/),
    );
  }
});
