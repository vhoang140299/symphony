import { z } from "zod";
import type { Issue, Tracker } from "../domain.js";
...
const issueSchema = z.object({
  id: z.string().min(1),
  native_ref: z.record(z.string(), z.unknown()).nullable().default(null),
...
export class MemoryTracker implements Tracker {
  ...
  constructor(provider: Record<string, unknown> = {}) {
    const issues = z.array(issueSchema).parse(provider.issues ?? []);
    const identifiers = new Set<string>();
    for (const issue of issues) {
      if (this.#issues.has(issue.id)) throw new Error(`Duplicate memory issue id: ${issue.id}`);
      if (identifiers.has(issue.identifier)) {
        throw new Error(`Duplicate memory issue identifier: ${issue.identifier}`);
      }
