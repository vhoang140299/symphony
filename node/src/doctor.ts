...
import { parseCodexRuntimeOptions, resolveSafeCodexHome } from "./agents/codex.js";
...
import { validateTrackerProvider } from "./trackers/registry.js";
import { inspectWorkspaceRoot } from "./workspace/manager.js";
...
async function inspectDefaultCodexExecutable(): Promise<void> {
  await requireOneExecutable([
    fileURLToPath(new URL("../../codex-symphony-wrapper.mjs", import.meta.url)),
    fileURLToPath(new URL("../codex-symphony-wrapper.mjs", import.meta.url)),
  ]);
...
async function requireOneExecutable(candidates: string[]): Promise<void> {
  for (const candidate of candidates) {
    try {
      await requireExecutable(candidate);
      return;
    } catch {
      // Source tests and compiled CLI have different module-relative roots.
    }
  }
